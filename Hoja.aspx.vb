...
Paint:
    ...
    ' Handler para los toggles de Hoja de Trabajo
    Protected Sub gvHT_RowCommand(sender As Object, e As GridViewCommandEventArgs)
        If e.CommandName <> "Toggle" Then Exit Sub

        Dim args() As String = e.CommandArgument.ToString().Split("|"c)
        If args.Length <> 3 Then Exit Sub

        Dim refId As Integer
        If Not Integer.TryParse(args(0), refId) Then Exit Sub

        Dim field As String = args(1)
        Dim val As String = args(2)

        ' Verificar si las 3 validaciones están completas
        Dim hf As HiddenField = TryCast(FindControlRecursive(Me, "hfHTValidado"), HiddenField)
        If hf IsNot Nothing AndAlso hf.Value = "1" Then
        ...
        Using cn As New SqlConnection(cs)
            ...
            Using cmd As New SqlCommand(sql, cn)
                ...
                If field = "autorizado" Then
                    cmd.Parameters.Add("@val", SqlDbType.Int).Value = CInt(val)
                Else
        ...
        Dim up As UpdatePanel = TryCast(FindControlRecursive(Me, "upHTGrids"), UpdatePanel)
        If up IsNot Nothing Then up.Update()
    End Sub
