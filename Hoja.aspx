    ...
    <script>
        (function () {
            ...
            function reqFs(el) {
                ...
                if (fn) fn.call(el); else window.open(img.src, '_blank');
            }
            function exitFs() {
            ...
            function isInFsWith(el) {
                const fs = fsElement();
                return !!fs && (fs === el || (fs.contains && fs.contains(el)));
            }
            ...
            function reset() {
                ...
                img.classList.remove('dragging', 'zooming', 'fs');
                img.style.transform = '';
            }
            function clampPan() {
                const vw = img.clientWidth, vh = img.clientHeight;
            ...
            function updateFsClass() {
                img.classList.toggle('fs', !!fsElement());
            }
            document.addEventListener('fullscreenchange', () => { updateFsClass(); reset(); });
            document.addEventListener('webkitfullscreenchange', () => { updateFsClass(); reset(); });

            // ---- Zoom con rueda (solo en FS) ----
            img.addEventListener('wheel', (e) => {
                if (!isInFsWith(img)) return;
                e.preventDefault();
                ...
                if (newScale !== scale) {
                    ...
                    tx = cx - (cx * newScale / scale) + tx;
                    ty = cy - (cy * newScale / scale) + ty;
                    scale = newScale;
                    clampPan(); apply();
                }
            }, { passive: false });
            ...
            img.addEventListener('mousedown', (e) => {
                if (!isInFsWith(img) || scale <= 1) return;
            ...
            img.addEventListener('touchmove', (e) => {
                ...
                if (e.touches.length === 1 && scale > 1) {
                    ...
                    scale = Math.max(MIN, Math.min(MAX, startScale * (d / tStartDist)));
                    clampPan(); apply();
                }
            }, { passive: false });
            ...
            // Nota: quitamos el "click para salir de FS" para no interferir con pan/zoom.
        })();
    </script>
