...
Public Class Hojalateria
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As EventArgs) Handles Me.Load
        ...
        AddHandler btnAutorizarHoj1.Click, AddressOf btnAutorizarHoj1_Click
        AddHandler btnAutorizarHoj2.Click, AddressOf btnAutorizarHoj2_Click
        AddHandler btnAutorizarHoj3.Click, AddressOf btnAutorizarHoj3_Click
    End Sub
    ...
    Protected Sub gvSust_RowDataBound(sender As Object, e As GridViewRowEventArgs) Handles gvSust.RowDataBound
        ...
        Dim virtualFolder As String = CombineVirtual(GetCarpetaRelVirtual(exp), SUBFOLDER_HOJA)
    ...
    Private Sub LoadAdmins()
        ...
        BindAdminDDL(ddlAutHoj1, dt)
        BindAdminDDL(ddlAutHoj2, dt)
        BindAdminDDL(ddlAutHoj3, dt)
    End Sub
    ...
    Private Sub btnAutorizarHoj1_Click(sender As Object, e As EventArgs)
        HandleAuthorization(ddlAutHoj1, txtPassHoj1, "authoj1", litAutHoj1)
    End Sub
    Private Sub btnAutorizarHoj2_Click(sender As Object, e As EventArgs)
        HandleAuthorization(ddlAutHoj2, txtPassHoj2, "authoj2", litAutHoj2)
    End Sub
    Private Sub btnAutorizarHoj3_Click(sender As Object, e As EventArgs)
        HandleAuthorization(ddlAutHoj3, txtPassHoj3, "authoj3", litAutHoj3)
    End Sub
    ...
    Private Function UpdateAdmisionAuth(expediente As String, fieldName As String, value As Boolean, authName As String) As Boolean
        ...
        Dim nameField As String = fieldName & "nombre" ' authoj1 -> authoj1nombre
    ...
    Private Sub PaintAutFlags()
        Dim expediente = GetExpediente()
        If String.IsNullOrWhiteSpace(expediente) Then
            litAutHoj1.Text = "" : litAutHoj2.Text = "" : litAutHoj3.Text = ""
            Exit Sub
        End If
        ...
        Using cn As New SqlConnection(CS)
            Using cmd As New SqlCommand("
                SELECT TOP 1
                    ISNULL(authoj1,0) AS authoj1, ISNULL(authoj1nombre,'') AS authoj1nombre,
                    ISNULL(authoj2,0) AS authoj2, ISNULL(authoj2nombre,'') AS authoj2nombre,
                    ISNULL(authoj3,0) AS authoj3, ISNULL(authoj3nombre,'') AS authoj3nombre
                FROM dbo.Admisiones WHERE Expediente=@exp;", cn)
                cmd.Parameters.AddWithValue("@exp", expediente)
                cn.Open()
                Using rd = cmd.ExecuteReader()
                    If rd.Read() Then
                        a1 = Convert.ToBoolean(rd("authoj1")) : n1 = Convert.ToString(rd("authoj1nombre"))
                        a2 = Convert.ToBoolean(rd("authoj2")) : n2 = Convert.ToString(rd("authoj2nombre"))
                        a3 = Convert.ToBoolean(rd("authoj3")) : n3 = Convert.ToString(rd("authoj3nombre"))
                    End If
                End Using
            End Using
        End Using
        ...
        litAutHoj2.Text = If(a2,
            "<span class='badge bg-success me-2'>Autorizado</span><span class='text-success'><i class='bi bi-person-check me-1'></i>" & s2 & "</span>",
            "<span class='badge bg-secondary me-2'>Pendiente</span><span class='text-muted'><i class='bi bi-person me-1'></i>—</span>"
        )
        litAutHoj3.Text = If(a3,
            "<span class='badge bg-success me-2'>Autorizado</span><span class='text-success'><i class='bi bi-person-check me-1'></i>" & s3 & "</span>",
            "<span class='badge bg-secondary me-2'>Pendiente</span><span class='text-muted'><i class='bi bi-person me-1'></i>—</span>"
        )

        ddlAutHoj1.Enabled = Not a1 : txtPassHoj1.Enabled = Not a1 : btnAutorizarHoj1.Enabled = Not a1
        ddlAutHoj2.Enabled = Not a2 : txtPassHoj2.Enabled = Not a2 : btnAutorizarHoj2.Enabled = Not a2
        ddlAutHoj3.Enabled = Not a3 : txtPassHoj3.Enabled = Not a3 : btnAutorizarHoj3.Enabled = Not a3
    End Sub
    ...
    Public Shared Function HasMinFotos(expediente As String, id As Integer, descripcion As String, min As Integer) As Boolean
        Try
            ...
            Dim virtualFolder As String = CombineVirtualStatic(carpetaRelVirt, SUBFOLDER_HOJA)
    ...
    Protected Sub btnDescargarPDF_Click(sender As Object, e As EventArgs)
        ...
        Using ms As New MemoryStream()
            ...
            Dim titleCell As New PdfPCell(New Phrase("Diagnóstico – Hojalatería", fontTitle))
            ...
            Response.AddHeader("Content-Disposition", $"attachment; filename=Hojalateria_{expediente}_{DateTime.Now:yyyyMMdd}.pdf")
